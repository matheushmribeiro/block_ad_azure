...
Write-Output "iniciando o script: ATESTADO DE USUARIOS" (Get-Date) | Out-File -FilePath $filepath -Append

#Set the Veriables for Credentials for MSol
...
{  
    ...
    #Desabilita o usuario
     ...
     Write-Output "Operação De desbloqueio No AD:" | Out-File -FilePath $filepath -Append
     Enable-ADAccount -Credential $credad $Loginad -Verbose 4>&1 | Out-File -FilePath $filepath -Append 

#print dos logins bloqueados

Write-Output "Usuario desbloqueado No Azure:" $login,$Data,$Loginad | Out-File -FilePath $filepath -Append
Write-Output " 
...
$csv | ConvertTo-Csv -Delimiter "," -NoTypeInformation  | foreach { $_.Replace('"','') } | Out-File -FilePath $saida
